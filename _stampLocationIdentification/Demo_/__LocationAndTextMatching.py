# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 22:56:36 2021
...
image = cv2.imread('image/normal/1 (18).jpg')  
...
if(len(stamps)==1):
    ...
    rect,crop_img = cv2.threshold(crop_img,10,255,cv2.THRESH_BINARY )
    ...
    crop_img = cv2.rectangle(crop_img, (np.min(temp.left),np.min(temp.top) ), 
                             ( np.max(temp.left)+np.max(temp.width)   , np.max(temp.top)+np.max(temp.height)), (115, 24, 165), 10)
    ...
    cv2.imwrite("header_image.png",header_image)
    ...
    cv2.imshow("crop_img2", cv2.resize(image_withstampLocator, (int(image_withstampLocator.shape[1]*1.5*scale_percent),
                                                int(image_withstampLocator.shape[0]*1.5*scale_percent)), interpolation = cv2.INTER_AREA) )
    cv2.waitKey(0)
    cv2.destroyAllWindows() 
