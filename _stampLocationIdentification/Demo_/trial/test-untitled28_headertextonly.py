...
frame_original=cv2.imread('image/normal/1 (19).jpg')
...
header_image = cv2.rectangle(header_image, (np.min(temp.left),np.min(temp.top) ), 
                             ...
                             # ( np.max(temp.left)+int(np.average(temp.width) )  , np.max(temp.top)+int(np.average(temp.height))), (120, 255, 0), 10)
...
# # image = cv2.imread('image/normal/1 (29).jpg')  
...
# frame_original=cv2.imread('image/normal/1 (1).jpg')
# #configuring parameters for tesseract
# custom_config = r'--oem 3 --psm 6  ' #-ctessedit_char_blacklist= 0123456789
...
# # header_image = cv2.dilate(header_image, kernel, iterations = 1) 
# # header_image = cv2.erode(header_image, np.ones((3,3),np.uint8), iterations = 1) 
