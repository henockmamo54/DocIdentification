# -*- coding: utf-8 -*-
"""
Created on Thu Jan  7 17:01:26 2021
...
frame = cv2.imread('../TrainTestData/Train/NotNormal/1 (1).jpg')  
...
for i in range(len(lines)-100):   
    ...
    if( (diff>50 ) or (abs(int(c[4])-int(n[2])) > 100)  ):
        print("************* draw", (int(b[1]), int(b[2])), (int(c[3]), int(c[4])),)
        img = cv2.rectangle(crop_img, 
                        (int(b[1]), h-int(b[2])), 
                        (int(c[3]), h-int(c[4])),
                        (0, 100, 120), 5)
