...
for file in filenames:        
    ...
    if(len(horizontalbars)!=1):
        ...
        imagebelowthebar = cv2.erode(imagebelowthebar, np.ones((3,3),np.uint8), iterations = 1)
        ...
        for i in range( boxes_array.shape[0]-1):
            ...
            temp_text=(temp_text.replace("전결","").replace("대결","").replace("결","").replace("전","")                  
                      )  
    ...
    if(isnormal):
        cv2.imwrite('cropped/notnormal/normal/'+file,imagebelowthebar)
    else:
        cv2.imwrite('cropped/notnormal/notnormal/'+file,imagebelowthebar)
